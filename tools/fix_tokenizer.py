...
def fix_tokenizer_json(model_path: str, max_vocab_size: int):
    ...
    if verify_passed:
        ...
        print(f"\n⚠️  IMPORTANT: Skipping tokenizer reload to avoid infinite loop")
        print(f"   HuggingFace adds back special tokens with invalid IDs on reload")
        print(f"   The tokenizer.json file is already fixed and ready for GGUF conversion")
        print(f"   The GGUF converter reads tokenizer.json directly, not HuggingFace's internal state")
    else:
...
def resize_tokenizer():
    ...
    try:
        ...
        # CRITICAL: Check the ACTUAL max token ID in the loaded tokenizer (including special tokens)
        # HuggingFace adds special tokens with IDs beyond vocab_size
        ...
        print(f"Max token ID in loaded tokenizer: {max_token_id:,}")
        print(f"Required vocab size to accommodate all tokens: {required_vocab_size:,}")
        
        # Use the required vocab size (not tokenizer.vocab_size)
        tokenizer_vocab = required_vocab_size
